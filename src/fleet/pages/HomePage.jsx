...
import { Activity, BatteryCharging, FileText, Gauge, PackageCheck, Route, Truck } from "lucide-react";
import { ScopeHeader } from "../components.jsx";
import { alertCoreInfo, alertDescription, alertLabel, alertTone } from "../alerts.js";
import { getVehicleSecondaryStatus } from "../vehicle-status.js";
import { DispatchOverview } from "./DispatchPage.jsx";
...
export function HomePage({ counts, summary, alerts, vehicles, scopeSelection, onScopeChange, onQuickAction, onOpenVehicleList, onBusinessAction, onOpenAlerts, onOpenVehicle }) {
    ...
    <section className="fleet-content-page fleet-home-page" aria-label="首页">
      ...
      <div className="fleet-page-body fleet-home-body">
        <nav className="fleet-home-analysis-tabs" aria-label="首页分析维度">
          <button type="button" className={analysis === "operation" ? "active" : undefined} onClick={() => setAnalysis("operation")}>经营调度</button>
          <button type="button" className={analysis === "vehicle" ? "active" : undefined} onClick={() => setAnalysis("vehicle")}>车辆运行分析</button>
        </nav>

        {analysis === "operation" ? <OperationAnalysis counts={counts} onBusinessAction={onBusinessAction} /> : (
          <VehicleAnalysis
            counts={counts}
            summary={summary}
            alerts={alerts}
...
function OperationAnalysis({ counts, onBusinessAction }) {
  ...
  return <>
    ...
    <DispatchOverview onBusinessAction={onBusinessAction} />
    <section className="fleet-home-data-card" aria-labelledby="home-shipment-title">
      <div className="fleet-home-section-heading"><div><h2 id="home-shipment-title">昨日发货量 <small>（t）</small></h2></div></div>
    ...
    <section className="fleet-home-data-card fleet-home-rate-card" aria-labelledby="home-rate-title">
      <div className="fleet-home-section-heading"><div><h2 id="home-rate-title">任务单及时送达率 <small>（%）</small></h2></div></div>
...
function VehicleAnalysis({ counts, summary, alerts, vehicles, onQuickAction, onOpenVehicleList, onOpenAlerts, onOpenVehicle }) {
  ...
  const drivingCount = vehicles.filter((vehicle) => getVehicleSecondaryStatus(vehicle) === "行驶中").length;
  const chargingCount = vehicles.filter((vehicle) => getVehicleSecondaryStatus(vehicle) === "充电中").length;
  ...
  return <>
    <section className="fleet-home-vehicle-radar" aria-label="车辆运行分析">
      <div className="fleet-home-radar-metrics">
        <button type="button" onClick={() => onOpenVehicleList("all", "全部车辆")}><span>在管车辆</span><b>{counts.total}</b><small>辆</small></button>
        <button type="button" onClick={() => onOpenVehicleList("online", "在线车辆")}><span>在线</span><b className="online">{counts.online}</b><small>辆</small></button>
        <button type="button" onClick={() => onOpenVehicleList("offline", "离线车辆")} aria-label={`离线 ${offlineVehicles.length} 辆${abnormalOfflineCount ? `，${abnormalOfflineCount} 辆异常离线` : ""}`}>
        ...
        <button type="button" onClick={() => onOpenVehicleList("driving", "行驶中车辆")}><span>行驶中</span><b className="online">{drivingCount}</b><small>辆</small></button>
        <button type="button" onClick={() => onOpenVehicleList("charging", "充电中车辆")}><span>充电中</span><b className="muted">{chargingCount}</b><small>辆</small></button>
      </div>
      <div className="fleet-home-analysis-actions">
        ...
        <button type="button" onClick={() => onQuickAction("today-charging-logs")}><BatteryCharging aria-hidden="true" /><span>今日充电<b>{summary.electricity.toLocaleString()}<small> kWh</small></b></span></button>
      </div>
      <p>数据来源于TSP车联网平台</p>
    </section>
    <section className="fleet-home-insight-grid" aria-label="监控快捷分析">
      ...
      <button type="button" onClick={() => onQuickAction("driving-logs")}><Route aria-hidden="true" /><span><b>行驶记录</b><small>查看今日车辆行程</small></span></button>
    </section>
